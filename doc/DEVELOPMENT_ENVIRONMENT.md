...
본 문서는 영화관 예매 시스템(Spring Cinema App Ex) 프로젝트의 개발환경 설정을 위한 가이드입니다. 필수 도구의 버전, 설치 방법, 경로 설정을 정의합니다.
...
- **개발 (dev)**: H2 인메모리 DB (별도 설치 불필요)
- **운영 (prod)**: MySQL 8.0
- **Redis**: 개발/운영 모두 필요 (좌석 HOLD, 분산 락, Refresh Token 등)
...
### 2.2 Spring Boot 4.0.2

Spring Boot는 Gradle 의존성으로 관리되므로 별도 설치 불필요합니다.

- **버전**: 4.0.2 (Milestone)
- **설정 위치**: `build.gradle` → `id 'org.springframework.boot' version '4.0.2'`
- **저장소**: `https://repo.spring.io/milestone` (build.gradle에 이미 설정됨)
...
Node.js 설치 시 함께 포함됩니다.

- **버전**: 9.x 이상 (Node 18+ 기본 포함)
- **패키지 잠금 파일**: `frontend/package-lock.json` (lockfileVersion 3)

---

### 3.3 React, TypeScript, Vite

프로젝트 `frontend/` 디렉터리에서 npm으로 설치합니다.

| 패키지 | 버전 (package.json) | 용도 |
|--------|---------------------|------|
...
좌석 HOLD, 분산 락, Refresh Token 등에 필수입니다.
...
- [DOCKER_SETUP.md](./DOCKER_SETUP.md) - Docker 앱 환경 설정 (Backend/Frontend/Mobile)
- [SERVER_STARTUP_GUIDE.md](./SERVER_STARTUP_GUIDE.md) - 서버 구동 및 DB 설정
- [TROUBLESHOOTING.md](./TROUBLESHOOTING.md) - 문제 해결
- [doc/trouble/](trouble/) - 상세 트러블슈팅
