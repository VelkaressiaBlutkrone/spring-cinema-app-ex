...
- **작업**:
  - [ ] 로그인 사용자 표시: 상단바 또는 메인 탭에 사용자 아이디(또는 이름) 표시, 탭/버튼으로 "마이페이지" 진입.
  - [ ] 마이페이지 화면: "내 정보", "장바구니", "결제/예매 내역" 섹션 또는 탭 구성.
  - [ ] 내 정보: 회원 정보 조회/수정 API 연동. 비밀번호·이메일·연락처 수정 폼 및 저장 호출.
  - [ ] 장바구니: HOLD 목록 API 연동, 상영·좌석 표시, 결제하기/취소 동작.
  - [ ] 결제 내역: 예매·결제 API 연동하여 목록 표시.
...
- 영화관 예매 시스템에 시네마틱하고 몰입감 있는 디자인 적용
- 웹(React)과 앱(Flutter) 전반에 일관된 비주얼 및 인터랙션 경험 제공

### 1. 디자인 컨셉

#### 1.1 다크 모드 기본 (Dark Mode Base)

- [ ] 배경색: 깊이감 있는 블랙 또는 다크 네이비 (`#0a0a0a`) 사용
- [ ] 포스터 및 영화 영상이 돋보이도록 대비 강화
- [ ] 웹/앱 공통 다크 테마 기본 적용

#### 1.2 글래스모피즘 (Glassmorphism) UI

- [ ] 투명·흐릿한(blurred) 레이어를 활용한 카드·모달·네비게이션
- [ ] `backdrop-filter: blur()` (웹) / `ImageFilter.blur` (Flutter) 적용
- [ ] 현대적이고 고급스러운 시각적 일관성 확보

#### 1.3 그라데이션 및 포인트 컬러

- [ ] 네온 그린, 일렉트릭 블루, 셋셋 코랄 등 강렬한 포인트 컬러 활용
- [ ] CTA 버튼, 호버, 강조 요소에 그라데이션 적용
- [ ] 다이나믹하고 시네마틱한 색감 정의 (디자인 토큰/테마 파일화)

### 2. 레이아웃 및 핵심 기능 (Dynamic UI/UX)

#### 2.1 히어로 영상 & 3D 요소

- [ ] **웹**: 메인 페이지 접속 시 기대 영화 하이라이트 영상 배경 재생
- [ ] **웹/앱**: 3D 포스터 모델링 또는 입체감 있는 포스터 표현
- [ ] 영화 선택 시 시네마틱한 인트로 효과

#### 2.2 키네틱 타이포그래피 (Kinetic Typography)

- [ ] **웹**: 마우스 스크롤에 따라 영화 제목이 역동적으로 움직이거나 변형
- [ ] **앱**: 스크롤/제스처에 따른 타이틀 애니메이션
- [ ] 사용자 몰입도 향상을 위한 시각적 피드백

#### 2.3 수평 스크롤 (Horizontal Scroll) 섹션

- [ ] 상영 중인 영화를 가로형으로 배치 (앨범 넘기기 형태)
- [ ] 웹: `overflow-x: auto` / 스냅 스크롤, 앱: `ListView.builder` 또는 `PageView`
- [ ] 모바일/데스크톱 모두 직관적인 스와이프/드래그 지원

#### 2.4 실시간 좌석 확인 — 3D 시트 뷰 (3D Seat View)

- [ ] 좌석 위치와 화면 시야각을 미리 확인할 수 있는 3D 시각화
- [ ] 웹: Three.js / React Three Fiber 또는 2.5D 뷰 검토
- [ ] 앱: Flutter 3D 또는 시뮬레이션 뷰 구현
- [ ] 좌석 선택 시 해당 좌석에서의 화면 시야 미리보기

### 3. 상호작용 (Micro-Interactions)

#### 3.1 마우스/터치 호버 효과

- [ ] **웹**: 영화 포스터 호버 시 예고편 짧은 재생 또는 3D 회전
- [ ] **앱**: 롱프레스 또는 포커스 시 유사한 미리보기/애니메이션
- [ ] 포스터 확대·그림자·글로우 등 시각적 피드백

#### 3.2 좌석 선택 애니메이션

- [ ] 좌석 클릭 시 탄성(bouncy) 인터랙션 적용
- [ ] 선택/해제 시 부드러운 스케일·회전·색상 전환
- [ ] 웹: CSS `animation` / Framer Motion, 앱: `AnimatedContainer` / `flutter_animate`

#### 3.3 모핑(Morphing) 효과

- [ ] 예매 단계 전환 시 버튼이 다음 화면으로 자연스럽게 변형
- [ ] 공유 레이아웃 요소(Shared Element Transition) 활용
- [ ] 단계별 플로우의 연속성 강화

#### 3.4 Lightning Dark 모드

- [ ] 단순 검은 배경이 아닌 인터랙션 반응형 다크 모드
- [ ] 클릭·스크롤 시 빛이 반응하는 듯한 글로우·하이라이트 효과
- [ ] 커서/터치 위치 주변 그라데이션 또는 빛 확산 효과 (웹: CSS / Canvas, 앱: 커스텀 페인터)

### 4. 적용 범위 및 우선순위

| 적용 대상 | 우선순위 | 비고 |
|----------|---------|------|
| 웹 메인/영화 목록 | 높음 | 히어로, 수평 스크롤, 다크 테마 |
| 웹 좌석 선택 | 높음 | 3D 시트 뷰, 좌석 애니메이션 |
| 앱 메인/영화 목록 | 높음 | 다크 테마, 수평 스크롤 |
| 앱 좌석 선택 | 높음 | 좌석 bouncy, 3D 시트 뷰 |
| 결제/예매 완료 | 중간 | 모핑, 글래스모피즘 |
| 관리자 웹 | 낮음 | 선택 적용 (가독성 우선) |
