...
class MemberApiService extends ApiServiceBase {
  MemberApiService(super.client);

  /// GET /api/members/me - 본인 프로필 조회
  Future<MemberProfileModel> getProfile() async {
    final response = await client.get(apiPathMembersMe, useAuth: true);
    client.throwIfNotOk('getProfile', response);
  ...
  Future<void> updateProfile({
    ...
    final response = await client.patch(apiPathMembersMe, body: body, useAuth: true);
    parseVoidResponse(response, 'updateProfile');
  }

  /// GET /api/members/me/holds - 본인 HOLD(장바구니) 목록
  Future<List<MemberHoldSummaryModel>> getMyHolds() async {
    final response = await client.get(apiPathMembersMeHolds, useAuth: true);
    client.throwIfNotOk('getMyHolds', response);
