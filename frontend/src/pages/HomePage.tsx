...
function SectionTitle({ children }: SectionTitleProps) {
  return (
    <div className="mb-4">
      <span className="text-amber/50 uppercase tracking-[3px] text-[10px]">collection</span>
  ...
  return (
    <div className="py-8">
      ...
      >
        ...
        />
        <div
          className="absolute inset-0 bg-gradient-to-t from-noir-bg/90 to-transparent"
          aria-hidden
        />
        <motion.div
        ...
        >
          ...
          <div className="my-3 h-px w-16 bg-amber/40" aria-hidden />
          <p className="text-noir-text-muted">상영 중인 영화를 확인하고 편리하게 예매하세요.</p>
        </motion.div>
      </motion.section>
        ...
        >
          ...
          {stats && (
            <motion.section variants={slideUp}>
              ...
              <NoirCard>
                <div className="flex flex-wrap gap-6 text-noir-text-muted">
                  <span>
                    영화관 <strong className="stat-glow text-noir-text">{stats.theaterCount}</strong>개
                  </span>
                  <span>
                    상영관 <strong className="stat-glow text-noir-text">{stats.screenCount}</strong>개
                  </span>
                  <span>
                    오늘 상영{' '}
                    <strong className="stat-glow text-noir-text">{stats.todayScreeningCount}</strong>편
                  </span>
                </div>
              </NoirCard>
            </motion.section>
                ...
                >
                  {upcoming.map((m) => (
                    <li key={m.id} className="scroll-snap-item shrink-0">
                      <Link
                      to="/movies"
                      className="group flex flex-col overflow-hidden rounded-sm border border-noir-border bg-noir-surface transition-all duration-300 hover:border-amber/50 hover:shadow-[0_0_24px_rgba(232,168,73,0.2)] focus:outline-none focus:ring-2 focus:ring-amber/50 block w-[min(160px,28vw)]"
                    >
            ...
            <NoirCard>
              {isAuthenticated && reservations.length > 0 ? (
                <>
                  <ul className="mb-4 space-y-3">
                    {reservations.map((r) => (
                      <li
                        key={r.reservationId}
                        className="flex flex-wrap items-center justify-between gap-2 rounded-sm border border-noir-border bg-noir-surface px-4 py-3"
                      >
                        <span className="font-medium text-noir-text">{r.movieTitle}</span>
                        <span className="text-sm text-noir-text-muted">
                ...
                <div className="flex flex-col items-center gap-4 py-8 text-center">
                  <span className="text-5xl text-noir-text-muted/60" aria-hidden>
                    🎬
                  </span>
                  <p className="text-noir-text-muted">
